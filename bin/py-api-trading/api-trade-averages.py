...
def SMA(period, granularity, pair):
    conn = httplib2.HTTPSConnection("api-sandbox.oanda.com")
    url = ''.join(["/v1/candles?count=", str(period + 1), "&instrument=", pair,
...
def compareAndTrade(period, granularity, pair, account):
    conn = httplib2.HTTPConnection("api-sandbox.oanda.com")
